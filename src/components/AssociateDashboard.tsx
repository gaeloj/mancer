...
import { Associate, Transaction, Announcement, Assembly, Poll } from '../types';
...
export default function AssociateDashboard({ 
  ...
  const handleMonthlyFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      setPayMonthReceiptFile(reader.result as string);
  ...
  const handleVoteCast = (poll: Poll) => {
    ...
    if (associate.financialStatus !== 'Adimplente') {
      alert('Apenas associados adimplentes podem votar.');
      return;
    }
    ...
    const updatedPoll: Poll = {
      ...
      voters: [...poll.voters, associate.id]
    };
  ...
  const handleConfirmInstallment = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleUploadReceipt = (installmentNumber: number, file: File) => {
    const reader = new FileReader();
    reader.onloadend = () => {
      ...
      <header className="bg-[#111111] border-b border-white/5 sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-10 w-10 bg-gradient-to-tr from-blue-600 to-blue-700 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-blue-900/30">
              GA
            </div>
            <div>
              <h1 className="font-bold text-white text-sm sm:text-base leading-tight">Portal do Associado</h1>
              <p className="text-[9px] text-blue-500 font-bold uppercase tracking-wider">Gestão Simplificada</p>
            </div>
          </div>
        ...
        <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-xl p-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="flex flex-col sm:flex-row gap-2 shrink-0 w-full md:w-auto">
            <button
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 py-2.5 px-4 bg-[#1a1a1a] hover:bg-white/5 border border-white/10 text-gray-300 hover:text-white text-xs font-bold rounded-xl transition-all cursor-pointer"
            >
              <Edit className="h-4 w-4" />
              {isEditing ? 'Cancelar Edição' : 'Atualizar Contativo'}
            </button>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-xl p-5 flex flex-col justify-between space-y-4">
            <div className="flex items-start justify-between">
              ...
              <span className={`p-2 rounded-xl ${
                associate.status === 'Ativo' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
              }`}>
                {associate.status === 'Ativo' ? <CheckCircle2 className="h-5 w-5" /> : <XCircle className="h-5 w-5" />}
              </span>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-400 mt-2 leading-relaxed">
                {associate.status === 'Ativo' 
                  ? 'Você está em situação regular cadastral e possui livre acesso à sede e ao portal.' 
          ...
          <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-xl p-5 flex flex-col justify-between space-y-4">
            <div className="flex items-start justify-between">
              ...
              <span className={`p-2 rounded-xl ${
                associate.financialStatus === 'Inadimplente' ? 'bg-red-500/10 text-red-400 border border-red-500/20' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
              }`}>
                {associate.financialStatus === 'Inadimplente' ? <AlertTriangle className="h-5 w-5" /> : <DollarSign className="h-5 w-5" />}
              </span>
            </div>
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-0.5 rounded-full ${
                  associate.financialStatus === 'Inadimplente' ? 'bg-red-500/10 text-red-400' : 'bg-emerald-500/10 text-emerald-400'
                }`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
            {(() => {
              ...
              return monthsList.map(month => {
                ...
                const isPaid = !!trans;
                const isExempt = associate.monthlyFee === 0 || associate.memberType === 'Cliente';
                const pendingReceipt = associate.pendingMonthlyReceipts?.find(r => r.monthKey === month.key);
                const isPending = !!pendingReceipt;
                ...
                if (isExempt) {
                  ...
                  cardClass = "bg-gradient-to-b from-emerald-500/[0.03] to-transparent border-emerald-500/20 hover:border-emerald-500/30";
                  statusLabel = "Pago";
                ...
                } else if (month.key < '07') {
                  cardClass = "bg-gradient-to-b from-red-500/[0.03] to-transparent border-red-500/20 hover:border-red-500/30";
                    ...
                    <div className="space-y-1.5">
                      {isPaid && trans ? (
                        <div className="text-[9px] text-gray-400 font-medium">
                          <p className="truncate">Via: {trans.paymentMethod}</p>
                          <p>{dateToBRL(trans.date)}</p>
                        </div>
                      ) : isExempt ? (
                        <p className="text-[9px] text-gray-500 font-semibold italic">Isento de Taxa</p>
                      ) : isPending ? (
                        <p className="text-[9px] text-amber-400 font-semibold italic flex items-center gap-1.5 bg-amber-500/5 px-2 py-1 rounded border border-amber-500/10">
                          <Clock className="h-3 w-3 text-amber-500 animate-pulse animate-bounce" />
                          Aguardando
                        </p>
                      ) : (
                        <button
            ...
            <div className="space-y-4">
              ...
              {polls.length === 0 ? (
                ...
                <div className="space-y-4 max-h-[480px] overflow-y-auto pr-1">
                  {polls.map((poll) => {
                    const hasVoted = poll.voters.includes(associate.id);
                    const isAdimplente = associate.financialStatus === 'Adimplente';
                      ...
                      >
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/5 pb-2.5">
                          <div className="space-y-0.5">
                            ...
                            <span className="text-[10px] text-gray-500 block">
                              Encerramento em: {dateToBRL(poll.date)} às {poll.time}h
                            </span>
                          </div>
                          <div className="flex items-center gap-2">
                            {poll.status === 'Ativo' ? (
                            ...
                            {isAdimplente ? (
                              ...
                              <div className="p-3 bg-red-500/5 border border-red-500/10 rounded-xl flex items-start gap-3">
                                ...
                                <div className="text-xs">
                                  <span className="font-extrabold text-red-400 block uppercase text-[10px]">Voto Bloqueado</span>
                                  <span className="text-gray-400 leading-normal block mt-1">
                                    Apenas associados adimplentes possuem direito ao voto democrático. Regularize seu débito na tesouraria para liberar seu acesso às decisões coletivas.
                                  </span>
                                </div>
                              </div>
                            )}
                        ...
                        {/* Already Voted Standing - HIDDEN results in real-time */}
                        {poll.status === 'Ativo' && hasVoted && (
                          <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-xl flex flex-col items-center justify-center text-center gap-2 py-6">
                            <div className="h-10 w-10 bg-emerald-500/10 text-emerald-400 rounded-full flex items-center justify-center border border-emerald-500/20 mb-1">
                              <Check className="h-5 w-5" />
                            </div>
                            <h6 className="font-extrabold text-white text-xs sm:text-sm">Voto Computado!</h6>
                            <p className="text-[11px] text-gray-400 max-w-xs leading-normal">
                              Seu voto foi registrado e computado com sucesso. Os resultados parciais estão ocultos e o vencedor será revelado pela administração após o encerramento da votação.
                            </p>
                          </div>
                        )}

                        {/* Closed Poll Results */}
                        {poll.status === 'Encerrado' && (
                          <div className="space-y-4">
                            {/* Homologation Header */}
                            ...
                            {winnerOption && (
                              <div className="p-4 bg-gradient-to-br from-emerald-500/10 to-emerald-500/[0.02] border border-emerald-500/20 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3 relative overflow-hidden">
                                ...
                                <div className="bg-[#121212] border border-white/5 px-3 py-1.5 rounded-lg shrink-0 text-right">
                                  ...
                                  <span className="text-xs font-extrabold text-emerald-400 font-mono">{winnerOption.votes} votos ({totalVotes > 0 ? Math.round((winnerOption.votes / totalVotes) * 100) : 0}%)</span>
                                </div>
                              </div>
                            )}
          ...
          <div className="bg-[#111111] rounded-2xl border border-white/5 shadow-xl p-5 space-y-4 h-fit">
            ...
            {saveSuccess && (
              <motion.div 
                initial={{ opacity: 0, y: -5 }}
                animate={{ opacity: 1, y: 0 }}
                className="p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs rounded-xl font-semibold"
              >
            ...
            {!isEditing ? (
              ...
              <form onSubmit={handleSaveContactInfo} className="space-y-4 pt-1">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">E-mail de Contato</label>
                  <input
                    type="email"
                    required
                    value={email}
                    onChange={(e) => setEmail(e.target.value)}
                    className="w-full px-3 py-2 bg-[#1a1a1a] border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1">Endereço Completo</label>
                  <textarea
                    rows={3}
                    required
                    value={address}
                    onChange={(e) => setAddress(e.target.value)}
                    className="w-full px-3 py-2 bg-[#1a1a1a] border border-white/10 rounded-xl text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 resize-none"
                  />
                </div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 py-2 px-3 border border-white/10 text-gray-400 hover:bg-white/5 text-xs font-semibold rounded-lg"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="flex-1 py-2 px-3 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg flex items-center justify-center gap-1.5 cursor-pointer"
                  >
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          ...
          >
            ...
            <form onSubmit={handlePayMonthConfirm} className="p-6 space-y-4">
              ...
              <div className="p-3 bg-[#161616] border border-white/5 rounded-xl flex justify-between items-center">
                <span className="text-[10px] text-gray-500 font-bold uppercase">Valor da Contribuição:</span>
                <span className="font-extrabold text-emerald-400 font-mono">{formatBRL(associate.monthlyFee)}</span>
              </div>
